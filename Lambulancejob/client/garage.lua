    ...
    while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then 
			...
			if NordGarage <= Config.Marker.DrawDistance then
			    wait = 0
                DrawMarker(20, Config.Nord.garage.x, Config.Nord.garage.y, Config.Nord.garage.z, nil, nil, nil, 0, nil, nil, 0.3, 0.3, 0.3, Config.Marker.Color.R, Config.Marker.Color.G, Config.Marker.Color.B, 200)
				DrawMarker(1, Config.Nord.garage.x, Config.Nord.garage.y, Config.Nord.garage.z-0.99, nil, nil, nil, 0, nil, nil, 3.0, 3.0, 0.2, 255, 255, 255, 200)
			end

			if NordGarage <= 1.5 then
                wait = 0
            ...
            if SudGarage <= Config.Marker.DrawDistance then
			    wait = 0
                DrawMarker(20, Config.Sud.garage.x, Config.Sud.garage.y, Config.Sud.garage.z, nil, nil, nil, 0, nil, nil, 0.3, 0.3, 0.3, Config.Marker.Color.R, Config.Marker.Color.G, Config.Marker.Color.B, 200)
				DrawMarker(1, Config.Sud.garage.x, Config.Sud.garage.y, Config.Sud.garage.z-0.99, nil, nil, nil, 0, nil, nil, 3.0, 3.0, 0.2, 255, 255, 255, 200)
			end

			if SudGarage <= 1.5 then
                wait = 0
                RageUI.Text({ message = "Appuyez sur ~r~[E]~s~ pour ouvrir →→ ~r~Garage", time_display = 1 })
					if IsControlJustPressed(1,51) then
						for k,v in pairs(Config.Garage.voiture) do
